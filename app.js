(function () {
  ...
  function updateCompletedGamesCount() {
    ...
    const completedGames = games.filter((g) => {
      ...
      const totalChecks =
        ctData.totalChecks || g.cheesetracker_total_checks || 0;
    ...
    try {
      ...
      if (!res.ok) {
        if (res.status === 401) {
          AUTH.clearSession();
          alert(
            "Your session expired. Please log in again on the Players page."
          );
  ...
  async function addNewRole() {
    ...
    try {
      const payload = editingRoleOriginal
        ? {
            action: "updateRole",
            originalName: editingRoleOriginal,
  ...
  async function manageModerator(action) {
    ...
    try {
      ...
      if (!res.ok) throw new Error(data.error);
      alert(
        `Moderator ${action === "add" ? "added" : "removed"} successfully!`
      );
