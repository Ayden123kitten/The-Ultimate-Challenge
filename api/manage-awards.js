...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (action === "updateAward") {
      ...
      const { originalName, awardData } = req.body;
      if (!originalName || !awardData || !awardData.name) {
        return res.status(400).json({ error: "Missing award data" });
      }

      const idx = awards.awards.findIndex((a) => a.name === originalName);
      if (idx === -1) return res.status(404).json({ error: "Award not found" });

      // Prevent duplicate names
      if (
        awardData.name !== originalName &&
        awards.awards.some((a) => a.name === awardData.name)
      ) {
        return res
          .status(400)
      ...
      awards.awards[idx] = { ...awards.awards[idx], ...awardData };
      ...
      const putRes = await fetch(awardsApiUrl, {
        ...
        body: JSON.stringify({
          message: `Update award: ${awardData.name}`,
          content: newAwardsContent,
          sha: sha,
          branch: branch
        })
    ...
    if (action === "deleteAward") {
      ...
      awards.awards = awards.awards.filter((a) => a.name !== awardName);
      ...
      // Remove award references from players
